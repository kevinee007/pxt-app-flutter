...
class TokenTest extends _i1.GeneratedContract {
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<BigInt> _buySmoothingFee({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[2];
    assert(checkSignature(function, '138ea692'));
    final params = [];
    final response = await read(function, params, atBlock);
    return (response[0] as BigInt);
  }
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<BigInt> _sellSmoothingFee({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[8];
    assert(checkSignature(function, 'ed9cbd93'));
    final params = [];
    final response = await read(function, params, atBlock);
    return (response[0] as BigInt);
  }
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<BigInt> _smoothingTokensToSwap({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[10];
    assert(checkSignature(function, '432654c4'));
    final params = [];
    final response = await read(function, params, atBlock);
    return (response[0] as BigInt);
  }
  ...
  Future<String> launch(
      _i1.EthereumAddress routerAddress, BigInt avaxAmount, BigInt tokenAmount,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[35];
    assert(checkSignature(function, 'eea21274'));
    final params = [routerAddress, avaxAmount, tokenAmount];
    return write(credentials, transaction, function, params);
  }
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> removeLimits(
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[49];
    assert(checkSignature(function, '751039fc'));
    final params = [];
    return write(credentials, transaction, function, params);
  }
  ...
  Future<String> setBuyFee(
      BigInt buyTaxFee, BigInt buyLiquidityFee, BigInt buySmoothingFee,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[54];
    assert(checkSignature(function, '25519cf2'));
    final params = [buyTaxFee, buyLiquidityFee, buySmoothingFee];
    return write(credentials, transaction, function, params);
  }
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> setNodeManagement(_i1.EthereumAddress nodeManagement,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[58];
    assert(checkSignature(function, 'fb0a7494'));
    final params = [nodeManagement];
    return write(credentials, transaction, function, params);
  }
  ...
  Future<String> setSellFee(
      BigInt sellTaxFee, BigInt sellLiquidityFee, BigInt sellSmoothingFee,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[59];
    assert(checkSignature(function, '1d865c30'));
    final params = [sellTaxFee, sellLiquidityFee, sellSmoothingFee];
    return write(credentials, transaction, function, params);
  }
