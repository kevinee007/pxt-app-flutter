...
class LPToken extends _i1.GeneratedContract {
  LPToken(
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<BigInt> MINIMUM_LIQUIDITY({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[2];
    assert(checkSignature(function, 'ba9a7a56'));
    final params = [];
    final response = await read(function, params, atBlock);
  ...
  Future<BigInt> allowance(
      _i1.EthereumAddress $param0, _i1.EthereumAddress $param1,
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<BigInt> balanceOf(_i1.EthereumAddress $param4,
      {_i1.BlockNum? atBlock}) async {
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> burn(_i1.EthereumAddress to,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[7];
    assert(checkSignature(function, '89afcb44'));
    final params = [to];
    return write(credentials, transaction, function, params);
  }
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<GetReserves> getReserves({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[10];
    assert(checkSignature(function, '0902f1ac'));
    final params = [];
    final response = await read(function, params, atBlock);
    return GetReserves(response);
  }
  ...
  Future<String> initialize(
      _i1.EthereumAddress _token0, _i1.EthereumAddress _token1,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[11];
    assert(checkSignature(function, '485cc955'));
    final params = [_token0, _token1];
    return write(credentials, transaction, function, params);
  }
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<BigInt> kLast({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[12];
    assert(checkSignature(function, '7464fc3d'));
    final params = [];
    final response = await read(function, params, atBlock);
    return (response[0] as BigInt);
  }
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> mint(_i1.EthereumAddress to,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<BigInt> nonces(_i1.EthereumAddress $param9,
      {_i1.BlockNum? atBlock}) async {
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<BigInt> price0CumulativeLast({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[17];
    assert(checkSignature(function, '5909c0d5'));
    final params = [];
    final response = await read(function, params, atBlock);
    return (response[0] as BigInt);
  }
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<BigInt> price1CumulativeLast({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[18];
    assert(checkSignature(function, '5a3d5493'));
    final params = [];
    final response = await read(function, params, atBlock);
    return (response[0] as BigInt);
  }
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> skim(_i1.EthereumAddress to,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[19];
    assert(checkSignature(function, 'bc25cf77'));
    final params = [to];
    return write(credentials, transaction, function, params);
  }
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> swap(BigInt amount0Out, BigInt amount1Out,
      _i1.EthereumAddress to, _i2.Uint8List data,
      {required _i1.Credentials credentials,
      _i1.Transaction? transaction}) async {
    final function = self.abi.functions[20];
    assert(checkSignature(function, '022c0d9f'));
    final params = [amount0Out, amount1Out, to, data];
    return write(credentials, transaction, function, params);
  }
  ...
  Future<String> sync(
    ...
    assert(checkSignature(function, 'fff6cae9'));
    final params = [];
    return write(credentials, transaction, function, params);
  }
  ...
  /// Returns a live stream of all Sync events emitted by this contract.
  Stream<Sync> syncEvents({_i1.BlockNum? fromBlock, _i1.BlockNum? toBlock}) {
    final event = self.event('Sync');
    final filter = _i1.FilterOptions.events(
        contract: self, event: event, fromBlock: fromBlock, toBlock: toBlock);
    return client.events(filter).map((_i1.FilterEvent result) {
      final decoded = event.decodeResults(result.topics!, result.data!);
...
class GetReserves {
  GetReserves(List<dynamic> response)
      : reserve0 = (response[0] as BigInt),
        reserve1 = (response[1] as BigInt),
        blockTimestampLast = (response[2] as BigInt);

  final BigInt reserve0;

  final BigInt reserve1;

  final BigInt blockTimestampLast;
}
...
class Burn {
  Burn(List<dynamic> response)
      : sender = (response[0] as _i1.EthereumAddress),
        amount0 = (response[1] as BigInt),
        amount1 = (response[2] as BigInt),
...
class Swap {
  Swap(List<dynamic> response)
      : sender = (response[0] as _i1.EthereumAddress),
        amount0In = (response[1] as BigInt),
        amount1In = (response[2] as BigInt),
        amount0Out = (response[3] as BigInt),
        amount1Out = (response[4] as BigInt),
        to = (response[5] as _i1.EthereumAddress);
